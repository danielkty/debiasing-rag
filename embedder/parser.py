...
class TemplateParser(nn.Module):
    def __init__(self, tokenizer_path, templates, max_length=None, additional_special_tokens=None,):
    ...
    def _parse_template(self, template):
        ...
        for template_piece in template_pieces:
            if template_piece.startswith('{') and template_piece.endswith('}'):
                ...
                tokenized_pieces.append(self.tokenizer.encode(template_piece, add_special_tokens=False))
    ...
    def _tokenize_piece(self, x, template_piece):
        if isinstance(template_piece, str):
            ...
            tokenized_piece = self.tokenizer.encode(template_piece, add_special_tokens=False)
...
class TemplatePooling(nn.Module):
    def __init__(self, strategy='mean'):
        super(TemplatePooling, self).__init__()
